...
class DimensionalityReducer(BaseModule):
    ...
    ) -> tuple[pd.DataFrame | None, Finding | None]:
        """
        ...
        try:
            ...
            if zero_variance_cols:
                zero_var_finding = Finding(
                    level="INFO",
                    message=(
                        f"Removed constant (zero-variance) numeric columns before dimensionality reduction: "
            ...
            if final_numeric_count < 3:
                skip_finding = Finding(
                    level="INFO",
                    message=(
                        f"Dataset has fewer than 3 valid numeric features ({final_numeric_count}) "
            ...
            df_transformed = pd.DataFrame(
                processed_data, columns=valid_numeric_columns, index=df_numeric_valid.index
            )

            return df_transformed, zero_var_finding

        except Exception as e:
            raise DimensionalityReducerError(
                f"Failed to prepare data for dimensionality reduction. Reason: {e}"
            ) from e
    ...
    def _run_pca(self, processed_df: pd.DataFrame) -> tuple[pd.DataFrame, list[float]]:
        ...
        try:
            ...
            embeddings_df = pd.DataFrame(
                embedding_data, columns=embedding_cols, index=processed_df.index
            )
            ...
            raise DimensionalityReducerError(
                f"Failed to perform PCA transformation. Reason: {e}"
            ) from e
    ...
    def _run_tsne(self, processed_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            perplexity_val = min(
                self.config.dimensionality_reduction.tsne_perplexity, max(1, (n_samples - 1) / 3.1)
            )
            ...
            embeddings_df = pd.DataFrame(
                embeddings_data, columns=embedding_cols, index=processed_df.index
            )

            return embeddings_df
        except Exception as e:
            raise DimensionalityReducerError(
                f"Failed to perform t-SNE transformation. Reason: {e}"
            ) from e
    ...
    def _run_umap(self, processed_df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            n_samples = processed_df.shape[0]
            n_neighbors_val = min(
                self.config.dimensionality_reduction.umap_n_neighbors, n_samples - 1
            )
            ...
            raise DimensionalityReducerError(
                f"Failed to perform UMAP transformation. Reason: {e}"
            ) from e

    def _save_results(
        self, embeddings_df: pd.DataFrame, output_dir: Path | str, filename_prefix: str
    ) -> str:
        """
        ...
        try:
            ...
            embeddings_df.to_csv(
                file_path,
                index=(
                    not embeddings_df.index.equals(
                        pd.RangeIndex(start=0, stop=len(embeddings_df), step=1)
                    )
                ),
            )
            ...
            raise DimensionalityReducerError(
                f"Failed to save results to {str(file_path) if file_path else filename_prefix}. Reason: {e}"
            ) from e
