...
class Visualizer(BaseModule):
    ...
    def _save_plot(self, fig: plt.Figure, filename: str) -> str | Path | None:
        ...
        try:
            ...
            finding = Finding(
                level="WARNING", message=f"Failed to generate plot: {filename}. Reason: {e}"
            )
        ...
        if report.metadata.task == "supervised":
            if report.target_analysis and report.target_analysis.identified_target:
                ...
                if target_name in df.columns:
                    ...
                    logger.warning(
                        f"Supervised task specified but target '{target_name}' not found in DataFrame for hue."
                    )
                    finding = Finding(
                        level="WARNING",
                ...
                logger.warning(
                    "Supervised task specified but no target identified. Will attempt unsupervised clustering for hue."
                )
                finding = Finding(
                    level="INFO",
    ...
    def _perform_clustering(self, df: pd.DataFrame, report: AnalysisReport) -> pd.Series:
        ...
        try:
            identified_target = (
                report.target_analysis.identified_target if report.target_analysis else None
            )
            ...
            num_pipeline = Pipeline(
                steps=[
                    (
                        "imputer",
                        SimpleImputer(
                            strategy=self.config.dimensionality_reduction.imputation_method
                        ),
                    ),
            ...
            if actual_n_clusters < n_clusters_config:
                logger.warning(
                    f"K-Means n_clusters reduced to {actual_n_clusters} (number of samples)."
                )
    ...
    def _run_univariate_plots(
        ...
        for col_details in report.profile.column_details:
            ...
            try:
                ...
                finding = Finding(
                    level="ERROR", message=f"Failed to generate plot for {col_name}: {e}"
                )
                self.findings.append(finding)
                plt.close("all")
    ...
    def _run_bivariate_plots(
        self, df: pd.DataFrame, report: AnalysisReport, hue_data: pd.Series, hue_name: str
    ) -> dict[str, str | Path]:
        """
        ...
        hue_is_numeric = (
            pd.api.types.is_numeric_dtype(hue_data)
            and hue_data.nunique() > self.config.target_detection.max_categorical_cardinality
        )
        ...
        try:
            if report.metadata.task == "supervised":
                selected_features = self._get_top_correlated_features(
                    report, hue_name, all_features, top_n
                )
            else:
                selected_features = self._get_top_associated_features(
                    df, hue_data, all_features, top_n
                )
        except Exception as e:
            ...
            self.findings.append(
                Finding(
                    level="WARNING",
                    message=f"Failed to select top features for bivariate plots: {e}",
                )
            )
        ...
        for col in selected_features:
            ...
            try:
                ...
                if col.inferred_type == "numerical" and not hue_is_numeric:  # num - cat
                    ...
                    col.inferred_type in ["categorical", "boolean"] and not hue_is_numeric
                ):  # cat - cat
                    plot_func = self._plot_grouped_countplot
                    plot_func(ax, feature_data, hue_data, col.column_name, hue_name)
                elif (
                    col.inferred_type in ["categorical", "boolean"] and hue_is_numeric
                ):  # cat - num
                    plot_func = self._plot_violinplot
                ...
                logger.error(
                    f"Failed to generate bivariate plot for {col.column_name} vs {hue_name}: {e}"
                )
                self.findings.append(
                    Finding(
                        level="WARNING",
    ...
    def _run_correlation_visuals(self, report: AnalysisReport) -> dict[str, str | Path]:
        ...
        if not report.statistical_analysis.correlation_report:
            logger.info(
                "Correlation report not found. Skipping correlation heatmaps visualization."
            )
            finding = Finding(
                level="INFO",
    ...
    def _run_dimensionality_visuals(
        self, report: AnalysisReport, hue_data: pd.Series, hue_name: str
    ) -> dict[str, str | Path]:
        """
        ...
        if not dimensionality_report.items():
            logger.info(
                "Dimensionality analysis report not found. Skipping embeddings visualization."
            )
            finding = Finding(
                level="INFO",
    ...
    def _plot_histogram(self, ax: plt.Axes, data: pd.Series, title: str):
        ...
        if data.empty:
            ax.text(
                0.5,
                0.5,
                "No data to plot (all values are NaN)",
    ...
    def _plot_heatmap(self, ax: plt.Axes, matrix: pd.DataFrame, title: str):
        ...
        sns.heatmap(
            data=matrix, ax=ax, cmap="coolwarm", center=0, annot=True, fmt=".2f", linewidths=0.5
        )
        ax.set_title(title)

    def _plot_embedding_scatter(
        self, ax: plt.Axes, embeddings: pd.DataFrame, title: str, hue_data: pd.Series, hue_name: str
    ):
    ...
    def _plot_regplot(
        self, ax: plt.Axes, y_data: pd.Series, x_data: pd.Series, y_name: str, x_name: str
    ):
    ...
    def _plot_violinplot(
        self, ax: plt.Axes, x_data: pd.Series, y_data: pd.Series, x_name: str, y_name: str
    ):
        """
    ...
    def _plot_grouped_countplot(
        ...
        sns.barplot(data=counts, x=feature_name, y="count", hue=hue_name, ax=ax, order=order)
    ...
    def _get_top_correlated_features(
        ...
        target_name: str,
        all_features: list[ColumnDetails],
        top_n: int,
    ) -> list[ColumnDetails]:
    ...
    def _get_top_associated_features(
        self, df: pd.DataFrame, hue_data: pd.Series, all_features: list[ColumnDetails], top_n: int
    ) -> list[ColumnDetails]:
        """
        ...
        """
        numeric_features = [
            col.column_name for col in all_features if col.inferred_type == "numerical"
        ]
        categorical_features = [
            col.column_name
            for col in all_features
            if col.inferred_type in ["categorical", "boolean"]
        ]
        ...
        if numeric_features:
            ...
            valid_numeric_features = [
                col for i, col in enumerate(numeric_features) if variance[i] > 1e-6
            ]
            ...
            if X_numeric_final.shape[1] > 0:
                ...
                for i, col_name in enumerate(valid_numeric_features):
                    feature_stats.append(
                        {"name": col_name, "score": f_scores[i], "pvalue": p_values[i]}
                    )

        if categorical_features:
            X_categorical = df_aligned[categorical_features]
            X_cat_imputed = X_categorical.apply(
                lambda col: col.fillna(col.mode()[0] if not col.mode().empty else "missing")
            )

            valid_categorical_features = [
                col for col in categorical_features if X_cat_imputed[col].nunique() > 1
            ]
            if valid_categorical_features:
                X_cat_encoded = X_cat_imputed[valid_categorical_features].apply(
                    lambda col: OrdinalEncoder().fit_transform(col.to_frame())[:, 0]
                )
                ...
                if X_cat_encoded.shape[1] > 0:
                    ...
                    for i, col_name in enumerate(valid_categorical_features):
                        feature_stats.append(
                            {"name": col_name, "score": chi2_scores[i], "pvalue": p_values[i]}
                        )
