...
class Finding(BaseModel):
    level: Literal["INFO", "WARNING", "ERROR"] = Field(
        ..., description="Type/Severity of the finding"
    )
...
class RunMetadata(BaseModel):
    ...
    run_timestamp: datetime = Field(
        ...
        description="The ISO 8601 timestamp when the analysis was started.",
    )
    analysis_mode: Literal["full", "fast"] = Field(
        ..., description="The mode of the analysis ('full' or 'fast')."
    )
    task: Literal["supervised", "unsupervised"] = Field(
        ..., description="The task to be performed using the dataset (supervised or unsupervised)"
    )
    user_target: str | None = Field(None, description="The target provided by the user if any.")
    original_row_count: int = Field(
        0, description="Total row count of the full input file, set before sampling in fast mode."
    )
...
class DatasetStats(BaseModel):
    ...
    original_row_count: int = Field(
        ..., description="Total row count of the entire dataset regardless of analysis mode"
    )
    analyzed_row_count: int = Field(
        ...,
        description="Represents the count of (observations/instances) in sampled dataset incase of 'fast' analysis mode, otherwise it is just the full row count.",
    )
    column_count: int = Field(..., description="Total number of columns (features) in the dataset.")
    total_memory_usage_mb: float = Field(
        ..., description="The total memory usage of the entire dataset in megabytes (MB)."
    )
...
class ColumnDetails(BaseModel):
    ...
    inferred_type: str = Field(
        ...,
        description="The high-level inferred type (e.g., 'numerical', 'categorical', 'datetime', 'text', 'id').",
    )
    dtype: str = Field(
        ..., description="The actual pandas dtype (e.g., 'int64', 'float64', 'object')."
    )
    missing_values_count: int = Field(
        ..., description="The absolute count of missing or null (NaN) values."
    )
    missing_values_pct: float = Field(
        ..., description="The percentage of values in the column that are missing."
    )
    unique_values_count: int = Field(
        ..., description="The number of distinct, non-null values in the column."
    )
    unique_values_pct: float = Field(
        ..., description="The percentage of values in the column that are unique."
    )
    stats: dict[str, Any] = Field(
        ..., description="Descriptive statistics (mean, std, etc.) or value counts."
    )
    memory_usage_mb: float = Field(
        ..., description="The memory usage of the column in megabytes (MB)."
    )
...
class DatasetProfile(BaseModel):
    ...
    findings: list[Finding] | None = Field(
        [], description="A list to hold findings from any module-level failures, warnings, or info"
    )
...
class CandidateTarget(BaseModel):
    ...
    column_name: str
    ml_task: str = Field(
        ..., description="The suggested machine learning task ('classification' or 'regression')."
    )
    justification: str = Field(
        ..., description="A human-readable reason why this column is a good candidate."
    )
...
class TargetVariableAnalysis(BaseModel):
    ...
    detection_method: Literal["auto", "user_specified"] = Field(
        ..., description="How the target was chosen ('auto' or 'user_specified')."
    )
    candidate_targets: list[CandidateTarget]
    identified_target: str | None = Field(
        None, description="The column name chosen as the most likely target."
    )
...
class OutlierReport(BaseModel):
    ...
    method: str = Field(
        ..., description="The method used for detection (e.g., 'Modified Z-score', 'Z-score')."
    )
...
class CorrelationReport(BaseModel):
    ...
    numerical_numerical_path: str | Path | None = Field(
        None, description="Path to the Spearman correlation matrix for numeric pairs."
    )
    categorical_categorical_path: str | Path | None = Field(
        None, description="Path to the Cramér's V association matrix for categorical pairs."
    )
    categorical_numerical_path: str | Path | None = Field(
        None,
        description="Path to the Correlation Ratio association matrix for categorical-numeric pairs.",
    )
...
class StatisticalAnalysis(BaseModel):
    ...
    outlier_report: dict[str, OutlierReport] = Field(
        ..., description="A mapping of column names to their outlier reports."
    )
    multicollinearity_report: dict[str, float] | None = Field(
        default_factory=dict,
        description="A mapping of highly collinear features to their VIF scores.",
    )
...
class VisualizationOutput(BaseModel):
    ...
    univariate_plots: dict[str, str | Path] = Field(
        ..., description="Mapping of column names to their distribution plot paths."
    )
    bivariate_plots: dict[str, str | Path] = Field(
        ..., description="Mapping of plot types (e.g., 'target_vs_feature_x') to their paths."
    )
...
class Recommendation(BaseModel):
    ...
    category: str = Field(
        ...,
        description="The area of recommendation (e.g., 'Preprocessing', 'Feature Engineering', 'Modeling').",
    )
    description: str
...
class AnalysisReport(BaseModel):
    """
    The central data structure holding all findings from the Sleuth pipeline.
