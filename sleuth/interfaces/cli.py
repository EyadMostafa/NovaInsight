...
logger = get_logger("sleuth.interfaces.cli")
...
@click.argument(
    "file_path", type=click.Path(exists=True, dir_okay=False, readable=True, resolve_path=True)
)
def analyze(file_path, **kwargs):
    """
    Performs a comprehensive analysis on a given tabular dataset.
    """
    logger.info("Sleuth CLI started.")
    file_path_obj = Path(file_path)
    if kwargs.get("modules"):
        module_strings = [m.strip().upper() for m in kwargs["modules"].split(",")]
        kwargs["requested_modules"] = [
            Operator[m].value for m in module_strings if m in Operator.__members__
        ]
    else:
